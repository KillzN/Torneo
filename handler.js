...
export async function handler(chatUpdate) {
  ...
  /* Creditos a Otosaka (https://wa.me/584120320626) */
    ...
    case 'remove':
      if (chat.welcome && !chat?.isBanned) {
        ...
        for (const user of participants) {
          let pp = 'https://raw.githubusercontent.com/STREAMSHOP/Torneo/master/src/avatar_contact.png';
          try {
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        ...
        const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;STREAMSHOP 👑;;;\nFN:STREAMSHOP 👑\nORG:STREAMSHOP 👑\nTITLE:\nitem1.TEL;waid=584120320626:+51 992 621 601\nitem1.X-ABLabel:STREAMSHOP 👑\nX-WA-BIZ-DESCRIPTION:[❗] ᴄᴏɴᴛᴀᴄᴛᴀ ᴀ ᴇsᴛᴇ ɴᴜᴍ ᴘᴀʀᴀ ᴄᴏsᴀs ɪᴍᴘᴏʀᴛᴀɴᴛᴇs.\nX-WA-BIZ-NAME:STREAMSHOP 👑\nEND:VCARD`;
        await mconn.conn.sendMessage(nk.from, { contacts: { displayName: 'STREAMSHOP 👑', contacts: [{ vcard }] } }, { quoted: callmsg });
